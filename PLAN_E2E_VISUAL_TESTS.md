...
Per `.claude/skills/orchestrate/rules/authoring.md`, split by concern, not by file:
...
Each phase PR built and passed unit tests before merging (see CI status on each PR). The prescribed concern-based split from `.claude/skills/orchestrate/rules/authoring.md` is satisfied: no concern mixes unrelated file changes, no pure-refactor commits are bundled with functional changes.
