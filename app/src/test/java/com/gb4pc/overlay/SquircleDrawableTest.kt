...
class SquircleDrawableTest {
     ...
     */
    private fun rasteriseSuperellipse(w: Int, h: Int): MaskData {
    ...
    private fun buildVertices(w: Int, h: Int): List<Pair<Double, Double>> {
        ...
        return (0 until steps).map { i ->
            ...
            val cosT = Math.cos(theta); val sinT = Math.sin(theta)
    ...
    private fun isInsidePolygon(px: Double, py: Double, vertices: List<Pair<Double, Double>>): Boolean {
        var inside = false
        val n = vertices.size
        var j = n - 1
        for (i in 0 until n) {
            val xi = vertices[i].first; val yi = vertices[i].second
            val xj = vertices[j].first; val yj = vertices[j].second
            if ((yi > py) != (yj > py) &&
    ...
    fun `buildSuperellipsePath pixel count matches ShapeTemplates squircle within 5 percent`() {
        ...
        val relDiff = Math.abs(pathMask.pixelCount - templateMask.pixelCount).toDouble() /
