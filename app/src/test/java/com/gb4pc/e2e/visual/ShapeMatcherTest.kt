...
class ShapeMatcherTest {
    ...
    private fun isEdgePixel(
        ...
        fun at(
            px: Int,
            py: Int,
        ) = if (px in 0 until w && py in 0 until h) bits[py * w + px] else false
    ...
    private fun makeBlurred(
        ...
        fun at(
            px: Int,
            py: Int,
        ): Int = if (px in 0 until w && py in 0 until h && base.bits[py * w + px]) 1 else 0
    ...
    private fun assertNotClassifiedAs(
        label: String,
        mask: MaskData,
        excluded: Shape,
    ) {
    ...
    @Test fun `square 64x64 classifies as SQUARE`() = assertClassifiesAs("square 64x64", makeNoisy(Shape.SQUARE, 64, 64), Shape.SQUARE)
    ...
    @Test fun `square 256x256 classifies as SQUARE`() =
        assertClassifiesAs(
            "square 256x256",
            makeNoisy(Shape.SQUARE, 256, 256),
            Shape.SQUARE,
        )

    // ── circle tests ──────────────────────────────────────────────────────────

    @Test fun `circle 64x64 classifies as CIRCLE`() = assertClassifiesAs("circle 64x64", makeNoisy(Shape.CIRCLE, 64, 64), Shape.CIRCLE)
    ...
    @Test fun `circle 256x256 classifies as CIRCLE`() =
        assertClassifiesAs(
            "circle 256x256",
            makeNoisy(Shape.CIRCLE, 256, 256),
            Shape.CIRCLE,
        )
    ...
    @Test fun `squircle 64x64 classifies as SQUIRCLE`() =
        assertClassifiesAs(
            "squircle 64x64",
            makeNoisy(Shape.SQUIRCLE, 64, 64),
            Shape.SQUIRCLE,
        )
