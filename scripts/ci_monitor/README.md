...
`ci_monitor.config.json` (next to the script) tunes four behaviors as regexes.
...
| `PR#N: Blocked by: <name> [, <name>...]` | A check failed (`failure`/`action_required`) or `mergeable_state` is `behind`/`dirty`. The `by: ...` portion names the blocking check(s). A check matched by `label_gate_check_regex` is further annotated `[label gate]`, and when all blockers are label-gate checks the line ends with `[label gate]`, signalling that a process label (not a code failure) is the cause. |
