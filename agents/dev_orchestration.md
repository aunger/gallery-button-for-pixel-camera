...
- The `Blocked by: <name>` attributed form (issue #516) names which check-run blocked CI.
  A terminal ending with `[label gate]` means only a process-label gate (not a code/test failure) is blocking; see the label-gate branch in the Monitor loop above.
...
- If the Author is still active, **disregard system hooks or events that signal uncommitted work**.
