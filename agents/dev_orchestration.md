...
- **For long-running CI (more than a few minutes), gate the Reviewer on CI completion at the Orchestrator level.** The emulator suite in this repo takes 7–12 minutes, which exceeds both the Monitor default timeout (5 minutes) and the subagent active-turn time limit (~3–4 minutes of `sleep`-based polling). A Reviewer dispatched immediately will exit before CI resolves and never post its review. Instead, after dispatching the Programmer, call `mcp__github__subscribe_pr_activity` on the PR, wait for a `<github-webhook-activity>` CI event indicating the gates have resolved (success or failure), and *then* dispatch the Reviewer. The Orchestrator is always-awake with no turn time limit, so this is the only reliable wait. The Reviewer can then read current status and post immediately. For short CI runs (under ~3 minutes), the Reviewer's own polling loop (in `pr_participation.md`) is acceptable.
