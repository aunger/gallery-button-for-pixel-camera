...
- **Use foreground (the default) when you need the results in the same turn.** Do not set `run_in_background: true` for any sub-agent whose findings you will act on before returning.
...
- **Use `run_in_background: true` only for fire-and-forget work whose results are not needed in the current turn.** If you are unsure whether you will need the results, use foreground.
- **Never rely on a background sub-agent's output in the same turn.** Background sub-agents surface as task-notification events after you exit.
