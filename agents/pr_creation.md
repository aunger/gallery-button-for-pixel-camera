...
Do not append any attribution byline, session URL, or "Generated by Claude" footer to PR/issue descriptions or comments. No `---` divider followed by attribution text. No `Co-authored-by: Claude` trailer in PR bodies.

This prohibits writing one; it does not oblige anyone to remove one that landed.
`.github/workflows/strip-session-bylines.yml` removes them from issue and PR bodies, comments and reviews automatically, so a byline that gets through costs nothing to leave.
It does not reach commit messages, which is the one place a byline is worth avoiding carefully, since it cannot be corrected after merge.
