...
2. **Before filing any issues**, check whether the PR already has a verification-plan comment from a prior run.
   Search the PR's issue-comment stream, where step 5 posts it, for one that contains the exact HTML marker `<!-- gb4pc-verification-plan -->`.
...
3. For each item on the *before merging* list, do the following.
   ...
   where `{number}` is the current PR number and `{issue's internal id}` is the issue's internal id (the `id` field, not its number; obtain it from the issue-creation response for new issues, or from the fetch in step 2 for already-covered items).
